...
export default function InviteModal({ workspaceId, workspaceName, currentUserId, onClose }: Props) {
  ...
  const inviteLink = typeof window !== 'undefined' ? `${window.location.origin}/invite/` : ''
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      const { data, error: invErr } = await supabase
        .from('invites')
        .insert({ workspace_id: workspaceId, invited_by: currentUserId, email: email.trim(), role })
        .select()
        .single()
  ...
  async function copyLink() {
    // Get a generic invite token (create a blank one)
    const { data } = await supabase
      .from('invites')
      .insert({ workspace_id: workspaceId, invited_by: currentUserId, email: 'invite@link', role })
      .select('token')
      .single()
    if (data) {
  ...
  return (
    <div style={overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={modal}>
        ...
        <p style={{ color: '#72767d', fontSize: 14, marginTop: 4, marginBottom: 20 }}>Teammates can view and join all public channels.</p>

        {/* Share link */}
        <div style={{ background: '#2c2f33', border: '1px solid #3f4348', borderRadius: 8, padding: '12px 14px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#f2f3f5', marginBottom: 2 }}>Invite link</div>
            <div style={{ fontSize: 12, color: '#72767d' }}>Anyone with this link can join your workspace</div>
          </div>
          <button onClick={copyLink} style={{ background: copied ? 'rgba(46,182,125,.15)' : '#36393f', border: `1px solid ${copied ? '#2eb67d' : '#3f4348'}`, borderRadius: 6, padding: '7px 14px', color: copied ? '#4ade80' : '#b9bbbe', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600 }}>
        ...
        <form onSubmit={handleInvite} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {sent.length > 0 && (
            <div style={{ background: 'rgba(46,182,125,.1)', border: '1px solid rgba(46,182,125,.3)', borderRadius: 8, padding: '10px 14px' }}>
              <div style={{ fontSize: 12, color: '#4ade80', fontWeight: 600, marginBottom: 4 }}>Invites sent to:</div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            <button type="submit" disabled={loading} style={{ flex: 1, ...primaryBtn, opacity: loading ? .7 : 1 }}>
